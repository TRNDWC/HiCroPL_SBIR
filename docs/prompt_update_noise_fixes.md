...
3. Residual fusion in final feature space
- Previous behavior: prompted image/text features were fused with fixed CLIP features using a residual-style addition before final normalization.
- Effect: the learned branch output and frozen reference branch were mixed together, which could hide whether gains came from prompting or from fusion.
- Experimental change: remove the residual addition and keep prompted features as the final branch features, while still returning fixed features separately for inspection.
...
- `src/model_hicropl.py`
  - Removed residual fusion between prompted and fixed features in `_forward_branch(...)`.
  - Final `image_features` and `text_features` now come directly from the prompted branch outputs.
...
- Clearer measurement of the effect of prompting alone, without fixed-feature residual mixing.
