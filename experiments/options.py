...
parser.add_argument('--exchange_query_from_sketch', action='store_true', help='Clean two-variable ablation against --exchange_detach_source (NOT the gated --exchange_sketch_driven): reuses photo2sketch_net.forward() UNCHANGED (full replacement, keeps its own internal linear_q(Q) residual -- no forward_delta, no external gate). Only two things differ from --exchange_detach_source: (1) the LKP query is W_q^l . mean(cross_prompts_sketch[i]) -- a per-layer Linear over that layer\'s own current sketch prompt -- instead of the free parameter photo_proxy_token; (2) the cut moves from the LKP OUTPUT to its INPUT (cross_prompts_photo[i].detach() feeds the LKP), so the LKP keeps learning instead of being frozen. Everything else (Mapper call, replacement rule) is the same code path as the default mechanism, so results are directly comparable. Adds one Linear(768,768) per cross_layer layer (~7M params for cross_layer=12); NO gate, so the sketch prompt is NOT protected at step 0 the way --exchange_sketch_driven protects it (expect a similar init-destroying discontinuity, this flag isolates whether the query source alone helps regardless of that). Mutually exclusive with --exchange_sketch_driven, --exchange_detach_source, --exchange_no_lkp, --exchange_no_mapper, --exchange_free_source and --exchange_self_source.')
...
if opts.propagator_sketch_queried:
    for other in ('exchange_detach_source', 'exchange_detach_input', 'exchange_free_source',
