...
parser.add_argument('--exchange_photo_first', action='store_true', help='Flip visual exchange direction: [0,cross_layer)=photo->sketch, [cross_layer,vision_depth)=sketch->photo (the direction originally tried WITH a gate, which failed -- use this to test the same direction WITHOUT a gate). Default (flag off): [0,cross_layer)=sketch->photo, [cross_layer,vision_depth)=photo->sketch, the direction that gave +0.34 mAP.')
...
parser.add_argument('--lambda_leak', type=float, default=0.0, help='Weight (lam_leak) for L_leak: penalizes prompt drift that falls in the semantic subspace S (orthogonal to modality axis M). 0 = off. Advisor reference default: 4.0. Requires --text_depth=1.')
parser.add_argument('--lambda_par', type=float, default=0.0, help='Weight (lam_par) for L_par: penalizes the sketch-photo text gap drifting off the modality axis M (normalized to [0,1] by default). 0 = off. Advisor reference default: 6.0. Requires --text_depth=1.')
parser.add_argument('--leak_rank_m', type=int, default=2, help='Rank r_M of the modality axis M (top-r_m SVD directions of the sketch-photo gap, estimated from a broad ImageNet-1k vocabulary).')
parser.add_argument('--leak_subspace_var', type=float, default=0.90, help='Explained-variance threshold for the semantic subspace S (keep top components explaining this fraction of variance among broad-vocabulary embeddings, then orthogonalize against M).')
parser.add_argument('--leak_warmup_steps', type=int, default=500, help='Steps to linearly ramp L_leak/L_par from 0 to full strength, so the regularizer does not dominate before the main task loss stabilizes.')
