...
# ----------------------
parser.add_argument('--domain_specific_ln', action='store_true', help='Give the VISUAL tower two separate LayerNorm sets, one per branch, restoring the base model'"'"'s trainable set {v^s, v^p, l^s_theta, l^p_theta} (Sain et al., CVPR 2023, category-level). Every nn.LayerNorm under clip.visual is replaced by a DomainLayerNorm holding two deepcopies of the SAME pretrained tensor, routed by the active domain (photo for photo/negative/photo_aug, sketch for sketch/sketch_aug, and by `modality` at eval). Text tower, clip_aug and the Mapper/LKP LayerNorms stay shared. Adds 39,936 trainable params and nothing else; off by default, in which case not a single DomainLayerNorm is built.')
