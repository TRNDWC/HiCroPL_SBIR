...
parser.add_argument('--sketch_self_refine', action='store_true', help='Ablation (capacity-matched no-exchange control): requires --disable_exchange. cross_prompts_sketch[i] is passed through the existing photo2sketch_net module with query=key=value=cross_prompts_sketch[i] (pure per-layer self-attention, no LKP, no proxy, nothing from photo at all) and overwrites current_sketch_prompts[i], for i in range(cross_layer). Unlike --exchange_self_source (which still routes through the LKP/attn_pooling_photo_nets and a multi-layer proxy set), this skips both entirely -- photo2sketch_net becomes just one extra learned refinement block on sketch prompts, with photo2sketch_net now receiving real gradient (unlike plain --disable_exchange alone, where it is idle/dead weight). Adds/removes no parameters or modules; isolates whether disable_exchange\'s effective capacity (not raw param count) explains any gap vs the exchange configs.')
