...
parser.add_argument('--enable_text_to_visual', action='store_true', help='New mechanism (Mechanism B): same-domain text->visual deep-prompt injection using NEW, dedicated LKP+Mapper instances (AttentionPooling/CrossPromptAttention, same classes as the photo<->sketch exchange but separate weights): the 11 "deeper" layers (index 1..prompt_depth-1) of text_prompt_photo/text_prompt_sketch (the existing SimpleTextPromptLearner instances, also feeding L_ce) are compressed by a per-domain LKP into proxy tokens, then a per-domain Mapper mixes them with that domain\'s own current visual deep prompt (query) to produce the new visual deeper-layer values for that SAME domain (text_photo->photo, text_sketch->sketch; layer 0 / ctx_photo / ctx_sketch untouched). Mutually exclusive with the photo<->sketch exchange: requires --disable_exchange, and only applies within the default architecture (--no_prompt_learning / --use_text_visual_exchange must be off). Model construction raises immediately if both this flag and the photo<->sketch exchange (i.e. NOT --disable_exchange) are active at once.')
