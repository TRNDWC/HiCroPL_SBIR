...
parser.add_argument('--use_attr_loss', action='store_true', help='Adds two ArGue-inspired auxiliary losses (L_attr, L_neg -- see src/losses_hicropl.py:attribute_losses(), NOT a paper reimplementation, an adaptation for this task, see report). L_attr = CE from an auxiliary classifier built entirely from frozen, pre-computed seen-class attribute text embeddings (data/attr_emb.npy). L_neg = entropy-maximization over a frozen bank of non-discriminative attribute embeddings (data/neg_emb.npy). Does not add any trainable parameter; attr_emb/neg_emb are non-persistent, non-trainable buffers, .detach()\'d again at point of use. Requires data/attributes_seen.json + --attr_file/--neg_file to exist and be class-index-aligned with the training classnames (checked at CustomCLIP construction, raises if misaligned).')
parser.add_argument('--lambda_attr', type=float, default=1.0, help='Weight for L_attr (only used when --use_attr_loss).')
parser.add_argument('--lambda_neg', type=float, default=0.5, help='Weight for L_neg (only used when --use_attr_loss).')
parser.add_argument('--attr_branch', type=str, default='sketch', choices=['sketch', 'photo'], help='Which branch feature (v) the attribute losses are computed on. Applied to exactly ONE branch, never both at once. Only used when --use_attr_loss.')
parser.add_argument('--attr_random', action='store_true', help='Ablation (content control, Run R4): replace A (attr_emb) with a fixed-seed (20240101, does not perturb the global training RNG) random tensor of identical shape, also L2-normalized. Only used when --use_attr_loss. Isolates whether L_attr\'s benefit comes from actual attribute knowledge vs. merely having an extra frozen auxiliary classifier head.')
parser.add_argument('--attr_file', type=str, default='data/attr_emb.npy', help='Path to A, the (n_cls, M, D) frozen attribute embedding tensor (see tools/gen_attributes.py). Only used when --use_attr_loss.')
parser.add_argument('--neg_file', type=str, default='data/neg_emb.npy', help='Path to N, the (K, D) frozen non-discriminative/negative attribute embedding tensor (see tools/gen_neg_bank.py). Only used when --use_attr_loss.')
