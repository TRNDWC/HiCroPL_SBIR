...
parser.add_argument('--exchange_query_from_sketch', action='store_true', help='Clean two-variable ablation against --exchange_detach_source (NOT the gated --exchange_sketch_driven): reuses photo2sketch_net.forward() UNCHANGED (full replacement, keeps its own internal linear_q(Q) residual -- no forward_delta, no external gate). The LKP query is W_q^l . mean(cross_prompts_sketch[i]) -- a per-layer Linear over that layer\'s own current sketch prompt -- instead of the free parameter photo_proxy_token. BY DEFAULT the cut is at the LKP INPUT (cross_prompts_photo[i].detach() feeds the LKP), so the LKP keeps learning instead of being frozen -- this is the two-variable-ablation setting against --exchange_detach_source. Combinable with --exchange_detach_source: doing so moves the cut back to the LKP OUTPUT and freezes both the LKP and sketch_query_proj, reproducing --exchange_detach_source\'s original freeze/read-only semantics while keeping the sketch-derived query -- i.e. every row of the LKP/photo/bottleneck comparison matches --exchange_detach_source except the query source. See also --exchange_query_from_sketch_no_detach (removes the cut entirely) and --exchange_bottleneck_rank. Adds one Linear(768,768) per cross_layer layer (~7M params for cross_layer=12); NO gate, so the sketch prompt is NOT protected at step 0 the way --exchange_sketch_driven protects it. Mutually exclusive with --exchange_sketch_driven, --exchange_no_lkp, --exchange_no_mapper, --exchange_free_source and --exchange_self_source.')
parser.add_argument('--exchange_query_from_sketch_no_detach', action='store_true', help='Override for --exchange_query_from_sketch (no effect and an error without it): removes the built-in detach at the LKP input, so cross_prompts_photo is NO LONGER read-only -- sketch-side loss flows all the way back into it through the LKP, same as the plain no-flag baseline does for the free-parameter-query mechanism, but now for the sketch-queried one. Isolates whether letting that gradient feedback reach photo helps or hurts, holding the sketch-queried extraction itself fixed.')
parser.add_argument('--exchange_bottleneck_rank', type=int, default=0, help='Fallback capacity control for --exchange_query_from_sketch: route R (the concatenated LKP output, i.e. the k/v fed to photo2sketch_net) through a rank-r bottleneck W_down.W_up (Linear(768,r) -> Linear(r,768), no bias) before it reaches the Mapper. Restricts the photo->sketch information channel to r dimensions instead of 768, on the hypothesis that a smaller search space is easier for the Mapper to learn to use well within a short training budget. Default 0 = off (no bottleneck, R passes through unchanged -- bit-identical to not having this flag). Currently requires --exchange_query_from_sketch (not wired up for --exchange_sketch_driven). Adds 2*768*r params (e.g. ~49K for r=32) -- negligible next to the ~44M the base mechanism already trains.')
parser.add_argument('--propagator_sketch_queried', action='store_true', help='NEW, fully separate Photo->Sketch propagator -- a completely independent code path from the existing LKP/Mapper (attn_pooling_photo_nets/photo2sketch_net and every --exchange_* variant), with its OWN modules (q_proj, pool, mapper_update). Sketch queries photo layer-by-layer: q_l = W_q^l . mean(Z_l), photo enters detached (read-only) as key/value into a per-layer AttentionPooling (self.pool), the results are concatenated across layers into R, and mapper_update (a fresh CrossPromptAttention, called WITHOUT its own query residual) maps R into delta -- P_hat = Q + delta (or the gated variants below). Off by default: constructs NO new module and changes NOTHING when unset. Mutually exclusive with --exchange_detach_source, --exchange_detach_input, --exchange_free_source, --exchange_self_source, --mapper_single_scale, --sketch_self_refine, --sketch_self_refine_ln and --disable_exchange (the old architecture this replaces).')
parser.add_argument('--propagator_gate_mode', type=str, default='none', choices=['none', 'zero_init', 'learned'], help='Magnitude control for --propagator_sketch_queried (no effect otherwise; passing a non-default value without the flag is an error). "none" = P_hat = Q + delta, unconstrained. "zero_init" = same formula, but mapper_update\'s two residual-branch output projections (attn.out_proj and ffn.c_proj) are explicitly zero-initialised, so delta == 0 and P_hat == Q EXACTLY at step 0 -- no learnable gate. "learned" = P_hat = Q + tanh(gamma_l) * delta with gamma a per-layer nn.Parameter initialised to 0 (ReZero-style), so delta is scaled to 0 at step 0 but the scale itself is learned.')
parser.add_argument('--propagator_bottleneck_rank', type=int, default=0, help='Low-rank bottleneck for --propagator_sketch_queried (no effect otherwise; a nonzero value without the flag is an error). 0 = off. >0 (e.g. 16/32/64/128): R (the concatenated per-layer LKP output, [cross_layer, 768]) is routed through W_down (768->rank, no bias) then W_up (rank->768, no bias) before reaching mapper_update, restricting the photo->sketch information channel to `rank` dimensions.')
parser.add_argument('--exchange_linear_projection', action='store_true', help='Simplest baseline for the ablation table (MaPLe/CLIP-AT-style coupling): one nn.Linear(768,768) per cross_layer layer, applied directly to the photo prompt and REPLACING the sketch prompt at that layer. nn.Linear operates on the last dim, so it is applied per-token (same weight across the n_ctx tokens of a layer). No LKP, no attention, no query mechanism, no detach (gradient flows both ways between photo and sketch through the Linear, matching how MaPLe/CLIP-AT\'s own coupling function works). Replaces the WHOLE Photo->Sketch block; mutually exclusive with every other --exchange_*/--propagator_* flag and with --disable_exchange (this flag IS the mechanism).')
parser.add_argument('--exchange_sketch_retrieval', action='store_true', help='Per-token retrieval variant: the LKP query is cross_prompts_sketch[i] ITSELF (all n_ctx rows, no projection, no averaging) instead of a single compressed proxy, so AttentionPooling\'s own built-in residual (token_query + attn(...)) lets each sketch token independently pull what it needs from photo while keeping its own identity -- output shape stays [n_ctx, dim] per layer, NO compression to 1 token. Reuses the SAME attn_pooling_photo_nets and photo2sketch_net modules unmodified (no new nn.Module, no sketch_query_proj -- fewer trainable params than --exchange_query_from_sketch). Mapper is then called with query=cross_prompts_sketch RAW, key/value=R (concat of the n_ctx-wide LKP outputs across all cross_layer layers, cross_layer*n_ctx rows instead of cross_layer) -- sketch appears twice on the path to the output (once via the LKP residual, once via the Mapper\'s own linear_q(Q) residual). Photo always enters the LKP detached (built in, read-only, LKP stays trainable). Mutually exclusive with --exchange_sketch_driven, --exchange_query_from_sketch, --exchange_detach_source, --exchange_detach_lkp_input, --exchange_no_lkp, --exchange_no_mapper, --exchange_free_source, --exchange_self_source and --mapper_single_scale (the last because it assumes 1 row per layer in the k/v, which breaks under this design\'s n_ctx rows per layer). Requires default --n_proxy 1 (unused by this path).')
parser.add_argument('--exchange_sketch_driven', action='store_true', help='Redesign of the Photo->Sketch block around "photo only supplies knowledge, sketch decides what to take". Two coupled changes. (a) Sketch-queried extraction: the LKP query stops being a free parameter (photo_proxy_token is no longer assigned) and becomes W_q^l . mean(Z_l), a per-layer Linear over the mean of that layer\'s SKETCH prompt, so the sketch side chooses what to pull out of photo -- and cross_prompts_sketch gains a second gradient path (it is otherwise ~45x weaker than the photo side). The photo prompts enter the LKP detached, so photo stays read-only while the LKP still learns. (b) Gated delta injection: the Mapper is called through forward_delta (its own query residual removed) and the result is injected as P_hat_l = Z_l + tanh(gamma_l) . delta_l with a per-layer gamma initialised to ZERO, so at step 0 the sketch prompt is preserved exactly instead of being overwritten by a randomly initialised projection. Expect gamma to move first and the Mapper/LKP to start learning only once it leaves zero (ReZero behaviour, not a bug). Adds one Linear per layer plus one scalar per layer. Mutually exclusive with --exchange_detach_source, --exchange_detach_lkp_input, --exchange_no_lkp, --exchange_no_mapper, --exchange_free_source, --exchange_self_source and --mapper_single_scale.')
parser.add_argument('--exchange_detach_lkp_input', action='store_true', help='Variant of --exchange_detach_source that moves the gradient cut from the LKP OUTPUT to the LKP INPUT: cross_prompts_photo[i] is detached before being fed as key/value to attn_pooling_photo_nets[i]. The photo branch stays read-only exactly as with --exchange_detach_source (no sketch-side gradient reaches cross_prompts_photo), but the LKP and its proxy tokens now sit BELOW the cut and keep receiving gradient, so the compression is learned instead of being a frozen random projection. Isolates "the photo prompts must not move" from "the compressor must not learn", which the output-side cut conflates. Mutually exclusive with --exchange_detach_source, --exchange_no_lkp, --exchange_free_source and --exchange_self_source.')
parser.add_argument('--exchange_no_mapper', action='store_true', help='Component ablation of the Photo->Sketch block: keep the LKP, DROP the Knowledge Mapper. Layer i\'s photo proxy (shape [1, dim]) is broadcast over the n_ctx positions and REPLACES that layer\'s sketch prompt, instead of being mixed in by photo2sketch_net, which is not assigned and therefore contributes no trainable parameters. The update rule stays replacement, matching the full mechanism. Two consequences to state when reporting: (a) all n_ctx prompt tokens of a layer become identical (a single proxy vector), and (b) cross_prompts_sketch[0:cross_layer] no longer reaches the encoder and has no gradient path left, so it is frozen -- the sketch prompts at those layers are no longer learnable, and the photo proxy alone defines them. Requires --n_proxy 1. Mutually exclusive with --exchange_no_lkp and --mapper_single_scale.')
parser.add_argument('--exchange_no_lkp', action='store_true', help='Component ablation of the Photo->Sketch block: keep the Knowledge Mapper, DROP the LKP. The RAW photo prompts of every layer in the range are concatenated and fed to photo2sketch_net as key/value (cross_layer * n_ctx tokens instead of cross_layer compressed proxies), so no per-layer compression happens; attn_pooling_photo_nets and photo_proxy_token are not assigned and contribute no trainable parameters. Update rule stays replacement, so this is directly comparable to the full mechanism. Isolates what the LKP\'s compression buys. Mutually exclusive with --exchange_no_mapper, --mapper_single_scale, --exchange_free_source and --exchange_self_source.')
...
parser.add_argument('--lkp_perceiver_refine', action='store_true', help='LKP variant (Perceiver-style, arXiv:2103.03206): before attn_pooling_photo_nets[i] pools the layer\'s n_ctx raw photo prompt tokens with its query, run them through ONE extra self-attention + FFN block (new module TokenSelfRefine, src/hicropl.py) so the tokens exchange information with each other first -- de-duplicate, surface distinctive content -- instead of the query distilling directly from raw, un-mixed tokens. Does not change attn_pooling_photo_nets, photo2sketch_net, or any gate/injection logic; only the sequence fed into the existing pooling query is pre-processed. Adds one TokenSelfRefine per cross_layer layer (~2.4M params for hidden_size=768). Scoped to the Photo->Sketch block only (the Sketch->Photo block, when n_deep > 0, is unaffected). Mutually exclusive with --lkp_qformer/--lkp_slot_attention (each replaces the same call site differently) and with --exchange_no_lkp/--exchange_free_source/--exchange_self_source/--exchange_no_mapper/--exchange_sketch_driven (those skip, redefine, or bypass the call site this refines). Freely combinable with --exchange_detach_source/--exchange_detach_lkp_input/--exchange_query_from_sketch/--mapper_single_scale/--n_proxy/--proxy_init.')
parser.add_argument('--lkp_qformer', action='store_true', help='LKP variant (Q-Former-style, BLIP-2/arXiv:2301.12597): replaces AttentionPooling with QFormerPooling (src/hicropl.py) at the Photo->Sketch LKP call site -- same forward(token_query, sequence_key, sequence_value) signature, so no other code changes. Two cross-attention rounds into the layer\'s raw photo tokens, with one self-attention round over the query/proxy tokens themselves in between, so multiple proxy tokens (--n_proxy > 1) can specialize against each other instead of independently reading the same source; with --n_proxy 1 (default) the middle self-attention step is a no-op on a single token, so this variant is expected to matter mainly combined with --n_proxy > 1. Adds roughly 2x an AttentionPooling\'s params per layer plus one extra self-attention. Mutually exclusive with --lkp_perceiver_refine/--lkp_slot_attention and with the same --exchange_* flags as --lkp_perceiver_refine (see its help). Freely combinable with --exchange_detach_source/--exchange_detach_lkp_input/--exchange_query_from_sketch/--mapper_single_scale/--n_proxy/--proxy_init.')
parser.add_argument('--lkp_slot_attention', action='store_true', help='LKP variant (Slot Attention, Locatello et al. 2020/arXiv:2006.15055): replaces AttentionPooling with SlotAttentionPooling (src/hicropl.py) at the Photo->Sketch LKP call site -- same call signature. Key difference from ordinary attention: weights are normalized ACROSS the proxy/slot dimension (slots COMPETE for each source token) instead of across the source tokens, combined with a --lkp_slot_iters-step GRU-based iterative refinement (paper default 3). token_query seeds the slots\' initial value (so --proxy_init/--exchange_query_from_sketch still apply) instead of the paper\'s random init. With --n_proxy 1 (default) there is only one slot so the competitive softmax has nothing to compete against (reduces to a plain normalized average) -- expected to matter mainly combined with --n_proxy > 1, same caveat as --lkp_qformer. Mutually exclusive with --lkp_perceiver_refine/--lkp_qformer and with the same --exchange_* flags as --lkp_perceiver_refine (see its help). Freely combinable with --exchange_detach_source/--exchange_detach_lkp_input/--exchange_query_from_sketch/--mapper_single_scale/--n_proxy/--proxy_init.')
parser.add_argument('--lkp_slot_iters', type=int, default=3, help='Number of iterative slot-attention refinement steps for --lkp_slot_attention (default 3, matches the original paper). No effect without --lkp_slot_attention.')
...
parser.add_argument('--enable_text_to_visual', action='store_true', help='New mechanism (Mechanism B): same-domain text->visual deep-prompt injection using NEW, dedicated LKP+Mapper instances (AttentionPooling/CrossPromptAttention, same classes as the photo<->sketch exchange but separate weights). Matches the ORIGINAL HiCroPL CrossModalPromptLearner\'s T->I block at cross_layer=prompt_depth exactly (github.com/zzeoZheng/HiCroPL, trainers/hicropl.py): ALL prompt_depth layers of text_prompt_photo/text_prompt_sketch (the existing SimpleTextPromptLearner instances, also feeding L_ce) -- including layer 0 (self.ctx) -- are compressed by a per-domain LKP into proxy tokens, then a per-domain Mapper mixes them with that domain\'s own current visual prompt at every layer (query, including layer 0 / ctx_photo / ctx_sketch) to produce the new visual values for that SAME domain (text_photo->photo, text_sketch->sketch). No layer is left untouched, and no I->T (visual->text) reverse mapping exists -- text_prompt_photo/sketch keep training only through L_ce, same as always. Mutually exclusive with the photo<->sketch exchange: requires --disable_exchange, and only applies within the default architecture (--no_prompt_learning / --use_text_visual_exchange must be off). Model construction raises immediately if both this flag and the photo<->sketch exchange (i.e. NOT --disable_exchange) are active at once.')
parser.add_argument('--text_to_visual_photo_only', action='store_true', help='Mechanism B restricted to the PHOTO domain: only text_prompt_photo -> photo visual prompts is applied; sketch visual prompts stay as their own independently trained cross_prompts_sketch (as under --disable_exchange alone). Requires --enable_text_to_visual. Mutually exclusive with --text_to_visual_sketch_only. The sketch-side Mapper_B/LKP_B is still constructed (so the photo side gets the exact same init as in the both-domains run) but never assigned, so it adds no trainable params.')
parser.add_argument('--text_to_visual_sketch_only', action='store_true', help='Mechanism B restricted to the SKETCH domain: only text_prompt_sketch -> sketch visual prompts is applied; photo visual prompts stay as their own independently trained cross_prompts_photo (as under --disable_exchange alone). Requires --enable_text_to_visual. Mutually exclusive with --text_to_visual_photo_only. The photo-side Mapper_B/LKP_B is still constructed (so the sketch side gets the exact same init as in the both-domains run) but never assigned, so it adds no trainable params.')
...
if opts.exchange_query_from_sketch_no_detach and not opts.exchange_query_from_sketch:
    raise ValueError(
        "--exchange_query_from_sketch_no_detach has no effect without "
        "--exchange_query_from_sketch -- the run would silently ignore it and be a plain "
        "baseline run under a misleading name. Pass --exchange_query_from_sketch, or drop "
        "--exchange_query_from_sketch_no_detach."
    )
if not opts.propagator_sketch_queried:
    if opts.propagator_gate_mode != 'none':
        raise ValueError(
            f"--propagator_gate_mode {opts.propagator_gate_mode!r} has no effect without "
            "--propagator_sketch_queried -- the run would silently ignore it and be a plain "
            "baseline run under a misleading name. Pass --propagator_sketch_queried, or drop "
            "--propagator_gate_mode."
        )
    if opts.propagator_bottleneck_rank != 0:
        raise ValueError(
            f"--propagator_bottleneck_rank {opts.propagator_bottleneck_rank} has no effect without "
            "--propagator_sketch_queried -- same reasoning as --propagator_gate_mode above. Pass "
            "--propagator_sketch_queried, or drop --propagator_bottleneck_rank."
        )
if opts.propagator_sketch_queried:
    for other in ('exchange_detach_source', 'exchange_detach_lkp_input', 'exchange_free_source',
        ...
        if getattr(opts, other, False):
            raise ValueError(
                f"--propagator_sketch_queried is not combinable with --{other}: that flag belongs "
                "to the OLD LKP/Mapper architecture (attn_pooling_photo_nets/photo2sketch_net); "
                "the new propagator is a fully separate code path with its own modules and does "
                "not share, and is not affected by, any of the old architecture's ablation flags."
            )
    if opts.disable_exchange:
        raise ValueError(
            "--propagator_sketch_queried is not combinable with --disable_exchange: the new "
            "propagator is the Photo->Sketch mechanism itself, replacing the old "
            "VisualVisualPromptLearner exchange block -- --disable_exchange (which only turns "
            "the OLD mechanism off) is meaningless here and would not do what its name implies."
        )
if opts.exchange_linear_projection:
    for other in ('exchange_detach_source', 'exchange_detach_lkp_input', 'exchange_free_source',
        ...
        if getattr(opts, other, False):
            raise ValueError(
                f"--exchange_linear_projection is not combinable with --{other}: it replaces the "
                "WHOLE Photo->Sketch block with a single Linear per layer, no LKP/Mapper of any "
                "kind is built, so flags governing the old mechanism have no target."
            )
    if opts.disable_exchange:
        raise ValueError(
