...
parser.add_argument('--exchange_no_mapper', action='store_true', help='Component ablation of the Photo->Sketch block: keep the LKP, DROP the Knowledge Mapper. Layer i\'s photo proxy (shape [1, dim]) is broadcast-ADDED to that layer\'s n_ctx sketch prompt tokens instead of being mixed in by photo2sketch_net, which is not assigned and therefore contributes no trainable parameters. NOTE this also changes the update rule from replacement to addition -- unavoidable, since replacing n_ctx sketch tokens with a single proxy would discard cross_prompts_sketch entirely; state it when reporting. Requires --n_proxy 1. Mutually exclusive with --exchange_no_lkp and --mapper_single_scale.')
