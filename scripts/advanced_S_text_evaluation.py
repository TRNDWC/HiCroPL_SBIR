...
def visualize_heatmap(df, title, path, group_map):
    """Vẽ heatmap đã được sắp xếp theo nhóm."""
...
def visualize_tsne(df, title, path, group_map):
    """Vẽ t-SNE projection."""
    ...
    for i, class_name in enumerate(df.index):
        group = group_map.get(class_name, 'other')
...
def main():
    ...
    parser.add_argument('--groups-file', required=True, help="JSON file defining class groups.")
    ...
    for name in design_names:
        ...
        # Heatmap
        visualize_heatmap(df_reordered, f"Sorted Heatmap - {name}", viz_out_dir / "1_sorted_heatmap.png", class_to_group)
