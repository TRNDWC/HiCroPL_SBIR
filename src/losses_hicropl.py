...
def loss_fn_hicropl(args, features, model=None):
    ...
    # --- L6: Prompt Alignment Loss (photo_prompts - sketch_prompts) ---
    if model is not None and lambda_prompt_align > 0:
        photo_prompts, sketch_prompts = model.get_visual_prompts()
        loss_prompt_align = lambda_prompt_align * prompt_alignment_loss(photo_prompts, sketch_prompts)
    else:
        loss_prompt_align = torch.tensor(0.0).to(device)

    # Total loss (including L1 triplet that was missing!)
    total_loss = (loss_triplet + loss_cross_modal + loss_consistency + 
                  loss_ce + loss_ce_aug)
