...
class VisualVisualPromptLearner(nn.Module):
    ...
    def forward(self):
        ######## S->P mapping (shallow photo prompts) ########
        # Sketch guides photo ở shallow layers [0..cross_layer]
        ...
        proxy_sketch_tokens = []
        for i in range(self.cross_layer):
            sketch_proxy = self.attn_pooling_sketch_nets[i](
        ...
        updated_photo_prompts = updated_photo_prompts.view(
            self.cross_layer, -1, updated_photo_prompts.shape[-1]
        )
        for i in range(self.cross_layer):
            self.cross_prompts_photo[i].data.copy_(updated_photo_prompts[i])
        ######## S->P mapping end ########

        ######## P->S mapping (deep sketch prompts) ########
        # Photo guides sketch ở deep layers [cross_layer..prompt_depth]
        sketch_prompts = torch.cat(
            [self.cross_prompts_sketch[i].unsqueeze(0)
             for i in range(self.cross_layer, self.prompt_depth)], dim=0
        )
        # LKP: compress photo prompts thành proxy
        proxy_photo_tokens = []
        for i in range(self.cross_layer, self.prompt_depth):
            photo_proxy = self.attn_pooling_photo_nets[i - self.cross_layer](
        ...
        updated_sketch_prompts = updated_sketch_prompts.view(
            self.prompt_depth - self.cross_layer, -1, updated_sketch_prompts.shape[-1]
        )
        for i in range(self.cross_layer, self.prompt_depth):
            self.cross_prompts_sketch[i].data.copy_(updated_sketch_prompts[i - self.cross_layer])
        ######## P->S mapping end ########
