...
class VisualVisualPromptLearner(nn.Module):
    def __init__(self, cfg, clip_model_photo, clip_model_sketch):
        ...
        ######## knowledge mapper: photo2sketch only ########
        ...
        # LKP: photo pools for all layers
        attn_pooling_photo = AttentionPooling(hidden_size=p_dim, num_attention_heads=8)
        self.attn_pooling_photo_nets = _get_clones(attn_pooling_photo, self.prompt_depth)

        # Proxy tokens: photo proxy for all layers
        photo_proxy_token = torch.randn(1, p_dim, dtype=dtype)
        self.photo_proxy_tokens = nn.ParameterList(
            [nn.Parameter(photo_proxy_token.clone()) for _ in range(self.prompt_depth)]
        )
    ...
    def forward(self):
        ######## P->S mapping (all layers) ########
        # Photo guides sketch ở toàn bộ layers [0..prompt_depth-1]
        sketch_prompts = torch.cat(
            [self.cross_prompts_sketch[i].unsqueeze(0)
             for i in range(self.prompt_depth)], dim=0
        )
        ...
        updated_sketch_prompts = updated_sketch_prompts.view(
            self.prompt_depth, -1, updated_sketch_prompts.shape[-1]
        )
        for i in range(self.prompt_depth):
